...
base_conv = GATConv(fin, fout, heads=heads, concat=False)
start_time = time.time()  #开始时间
out = base_conv(h,a)
end_time = time.time()   #结束时间
print("time:%d"  % (end_time-start_time))
...
# 获得 edgeIndex
def getSequenceEdgeIndex(sequence,edge_radius):
	
	sequence_mask = []
	position_cube = sequence.data.numpy()
	
	for timestep in range(position_cube.shape[0]): # 总的时间步长上
		# pdist计算行人两两之间的距离 squareform并返回距离矩阵
		dists = squareform(pdist(position_cube[timestep], metric='euclidean')) # num_peds,num_peds

		# Put a 1 for all agent pairs which are closer than the edge_radius.
		adj_matrix = (dists <= edge_radius).astype(np.int8) # 将需要相连成边的记为1 领接矩阵num_peds,num_peds

		edge_indx = np.where(adj_matrix==1)
		sequence_mask.append(torch.from_numpy(np.stack(edge_indx,0)))

	return sequence_mask
	
a = torch.randint(0,10,size=(12,10,2))
print(getSequenceEdgeIndex(a,1))
"""
